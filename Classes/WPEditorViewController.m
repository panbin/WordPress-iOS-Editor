...
- (void)canmerTouchMediaOptions
{
    if (self.editorView.isInVisualMode) {
        if ([self.delegate respondsToSelector: @selector(editorDidPressCamerMedia:)]) {
            [self.delegate editorDidPressCamerMedia:self];
        }
    } else {
        // Do not allow users to insert images in HTML mode for now
        __weak __typeof(self)weakSelf = self;
...
{
    [self.editorView.titleField blur];
    [self.editorView.contentField blur];
}
